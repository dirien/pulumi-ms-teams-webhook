...
import (
	...
	"github.com/dirien/pulumi-ms-teams-webhook/internal/petname"
...
func Command() *cobra.Command {
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			name := petname.PetName(0)

			if region == "" {
				return fmt.Errorf("must specify an AWS region. See --help")
			}
			if msTeamsWebhookURL == "" {
				return fmt.Errorf("must specify a MS Teams Webhook URL. See --help")
			}
	...
	command.Flags().StringVar(&msTeamsWebhookURL, "ms-teams-webhook-url", "", "The URL of the MS Teams webhook to use.")
